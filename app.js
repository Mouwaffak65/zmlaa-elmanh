...
const App = {
  ...
  setupOnboarding() {
    ...
    // Handle Enter key in country input
    document.getElementById('onbCountry').addEventListener('keydown', (e) => {
      if (e.key === 'Enter') document.getElementById('nextStep2').click();
    });

    document.querySelectorAll('.level-option').forEach(btn => {
      btn.addEventListener('click', () => {
        document.querySelectorAll('.level-option').forEach(b => b.classList.remove('selected'));
    ...
    document.getElementById('finishBtn').addEventListener('click', async () => {
      ...
      if (!selected) {
        showToast('الرجاء اختيار مستوى الدراسة');
        return;
      }
      ...
      if (result.success) {
        ...
        showToast('حدث خطأ، الرجاء المحاولة مرة أخرى');
      }
    });
  },
  ...
  renderUserBadge() {
    ...
    badge.innerHTML = `
      <div class="user-info">
        ...
        <div class="detail">${u.country} · ${u.level}</div>
      </div>
      <div class="avatar" style="background:${u.avatar_color}">${initial}</div>
    `;
  },
  ...
  renderPosts() {
    const container = document.getElementById('postsContainer');
    const filtered = this.currentFilter === 'الكل'
      ? this.posts
    ...
    if (filtered.length === 0) {
      container.innerHTML = `
        <div class="empty-state">
          ...
          <p>كن أول من يشارك تجربته مع المنح الدراسية!</p>
        </div>`;
      return;
    }
    ...
    filtered.forEach(p => {
      ...
      likeBtn?.addEventListener('click', () => this.handleLike(p.id));

      const commentBtn = card.querySelector('.comment-btn');
      commentBtn?.addEventListener('click', () => this.toggleComments(p.id));

      const commentForm = card.querySelector('.comment-form');
      commentForm?.addEventListener('submit', (e) => {
        e.preventDefault();
        const input = commentForm.querySelector('input');
        if (input.value.trim()) this.addComment(p.id, input.value);
      });
    });
  },
  ...
  renderPostCard(p) {
    ...
    const initial = p.user_name?.charAt(0) || '?';
    const likedClass = p.liked ? 'liked' : '';
    return `
      <div class="post" id="post-${p.id}">
        <div class="post-header">
          <div class="avatar" style="background:${p.user_color || '#6C63FF'}">${initial}</div>
          <div class="post-author">
            <div class="name">${p.user_name}</div>
            <div class="country">${p.user_country || ''}</div>
          </div>
  ...
  async submitPost() {
    ...
    if (!content) {
      showToast('الرجاء كتابة محتوى المنشور');
      return;
    }
  ...
  async toggleComments(postId) {
    ...
    list.innerHTML = comments.map(c => `
      <div class="comment">
        <div class="avatar" style="background:${c.user_color || '#6C63FF'}">${c.user_name.charAt(0)}</div>
        <div class="comment-body">
          <div class="name">${c.user_name}</div>
          <div class="text">${c.content}</div>
          <div class="time">${this.formatTime(c.created_at)}</div>
        </div>
      </div>
    `).join('');
  },
  ...
  async addComment(postId, content) {
    ...
    if (result.success) {
      ...
      const form = document.querySelector(`#comments-${postId} .comment-form input`);
      if (form) form.value = '';
    }
  },
  ...
  async loadMembers() {
    ...
    if (members.length === 0) {
      container.innerHTML = '<div class="empty-state"><h3>لا يوجد أعضاء بعد</h3></div>';
      return;
    }
    container.innerHTML = members.map(m => `
      <div class="member-card">
        <div class="avatar" style="background:${m.avatar_color || '#6C63FF'}">${m.name.charAt(0)}</div>
        <div class="name">${m.name}</div>
        <div class="country">${m.country}</div>
        <div class="level">${m.level}</div>
      </div>
    `).join('');
  },
  ...
  linkify(text) {
    ...
    return text.replace(urlRegex, url => `<a href="${url}" target="_blank" rel="noopener" style="color:var(--primary)">${url}</a>`);
  }
};
